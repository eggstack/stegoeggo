...
- **JPEG stego redundancy bug** (`steganography.rs:788-841`): Only first pass embeds when `redundancy > 1`. `embedded` counter not reset between passes. Fix: add `embedded = 0;` after line 840 (see `plans/plan.md` item 1A.1)
...
- **Error variants `Dimensions` and `JpegTranscode` are dead code**: Defined in `error.rs` but never constructed. `Dimensions` was intended for dimension exceeded errors but pipeline uses `Error::ImageDecode` instead. `JpegTranscode` was intended to wrap `TranscoderError` but `TranscoderError` is used directly instead.
