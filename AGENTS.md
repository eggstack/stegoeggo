...
- **JPEG pixel stego redundancy bug**: `embed_jpeg_stego` (`steganography.rs:788-841`) has a bug where only the first pass embeds when `redundancy > 1`. After pass 0 completes, `embedded >= bits_per_pass` causes immediate break on passes 1-4. The `embedded` counter must be reset to 0 after each pass completes. This bug affects the redundancy embedding feature — extraction with majority voting won't work correctly. Fix: add `embedded = 0;` after the inner loop break chain at line 840
...
- **Error variants `Dimensions` and `JpegTranscode` are dead code**: Defined in `error.rs` but never constructed. `Dimensions` was intended for dimension exceeded errors but pipeline uses `Error::ImageDecode` instead. `JpegTranscode` was intended to wrap `TranscoderError` but `TranscoderError` is used directly instead. Consider removing these
- **`From<TranscoderError> for Error` not implemented**: Transcoder errors lose specificity when bubbled up — `TranscoderError::HuffmanEncode` becomes generic `Error::ImageEncode`. Implementing `From<TranscoderError>` would preserve error semantics through the call stack
