...
- **Fuzz harness**: 12 targets in `fuzz/`: `pipeline_bytes`, `tiled_round_trip`, `jpeg_parser`, `payload_v3_parser`, `png_metadata`, `webp_riff_parser`, `xmp_extract`, `metadata_merge`, `detached_manifest_parse`, `detached_manifest_verify`, `provenance_canonicalize`, `verification_report`. Run with `cargo +nightly fuzz run <target> -- -max_total_time=60`. Add regression tests in `tests/robustness.rs` for findings
...
- **V3-first extraction**: All extraction paths (LSB non-tiled, LSB tiled, DCT non-tiled, DCT tiled, and all verification variants) use `classify_v3_probe()` as the shared v3 classification implementation. `classify_v3_probe` accepts an optional `&ResourceLimits` parameter for `max_payload_bytes` enforcement and returns `V3ProbeResult::ResourceLimitExceeded` when the payload exceeds limits. Non-tiled paths extract a 6-byte prefix first, classify, then extract exact `total_length`. Tiled paths extract a48-byte probe (covering full core header + auth tag), classify, then extract exactly `total_length` bits (not `max(total_bits, V3_PROBE_BITS)`). `V3ProbeResult` now includes deeper header validation: auth_algorithm, key_id_len, and header_length >= core + key_id. Only `NotV3` permits legacy v2/v1 fallback; `MalformedV3`, `UnsupportedVersion`, and `ResourceLimitExceeded` terminate without legacy decoding
...
- **OperationBudget enforcement methods**: `OperationBudget` now provides `check_and_observe_*` methods that enforce resource limits before observing work. Methods: `check_and_observe_png_chunk`, `check_and_observe_jpeg_segment`, `check_and_observe_webp_chunk`, `check_and_observe_xmp_bytes`, `check_and_observe_metadata_field`, `check_and_observe_tile_origin`, `check_and_observe_verification_seed`. Each enforces the relevant `ResourceLimits` field and returns `Err` on violation
- **Runtime warnings from embed outcomes**: `process_request_bytes_with_warnings` and `process_request_bytes_with_report` now derive runtime warnings (`LsbCapacitySkipped`, `DctCapacityInsufficient`, `ProgressiveJpegFallback`) from observed `EmbedOutcomeSummary` status and path, in addition to plan-level warnings
- **`largest_buffer_bytes()` on ResourceUsage**: Truthful naming for the largest individual buffer size observed during an operation. Returns `peak_allocations_bytes` (the field retains its name for backward compatibility)
- **RC workflow uses authoritative validation script**: `.github/workflows/release-candidate.yml` now calls `scripts/validate-release.sh --phase all` instead of maintaining an independent command list. RC requires a full40-character SHA input and verifies checkout equality
