...
- **Architecture review findings**: Several architecture docs had discrepancies — parallel threshold calculation wrong (should be 1c:4096, 4c:16384, 16c:65536), XorShiftRng uses `wrapping_add` not XOR, Protected/ diagram grouping doesn't exist (flat struct), verify_image_bytes DOES do DCT stego verification contrary to docs, CLI batch does NOT preserve directory structure. See `plans/*-review.md` for full details.
