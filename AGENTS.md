...
- **Tiled JPEG candidate identity** — `application_support` returns opaque `TiledJpegCandidateKey` values with each prefix. Root tiled extraction must reuse that exact key for V3 header/full and legacy lengths; invalid, NotV3, malformed, unsupported, and failed-integrity candidates continue to later bounded candidates. `max_origins` limits tile origins, not nearby seed/redundancy variants.
- **Tiled JPEG search context** — Each root tiled-JPEG extraction/verification call creates one operation-local carrier-owned `TiledJpegSearch`. It decodes the JPEG coefficient container once; prefix enumeration and all exact-key V3/V2/V1 extraction reuse its private state. The bounded work is `O(1 * full JPEG decode + candidate_count * block extraction)` rather than a full decode per candidate. The context is not cached or exposed through the default carrier API.
- **Tiled JPEG embed verification** — Tiled embedding records the first successful tile/local seed, encodes once, then decodes the encoded output and extracts that exact tile. `EmbedOutcome::Embedded` is reported only when the encoded output yields the original payload; failed post-encode verification is reported as a non-success outcome.
...
- **Generic carrier core** — `stegoeggo-stego/src/lsb.rs`, `stegoeggo-stego/src/jpeg.rs`, and `stegoeggo-stego/src/frame.rs` are the stable generic carrier API. Raw operations handle arbitrary bytes with caller-known lengths; `lsb::embed_framed`/`extract_framed` and `jpeg::embed_framed`/`extract_framed` compose the existing bounded frame format for recovery without the original length (or JPEG `actual_redundancy`). JPEG framed extraction probes only the configured redundancy down to 1 and accepts a candidate only after complete frame/CRC validation. `lsb::embed_in_place` mutates an owned `RgbaImage` and returns `InPlaceEmbedReport`; cloning `lsb::embed` and the in-place path share one corrected V2 mutation core, and corrected V2 bitstreams are read/written directly without one-byte-per-bit vectors. Low-level mechanics in `lsb_internal` and `jpeg_transcoder` are private implementation details. The parent rights-protection crate uses the carrier's narrow, unstable `application-support` operation layer; it does not expose parser, coefficient, permutation, or application-payload types. The carrier `EmbedReport` is generic over the output type (`RgbaImage` for LSB, `Vec<u8>` for JPEG) — pixel-domain no longer performs an implicit PNG encode
...
- **Tiled JPEG support boundary** — The optional parent-crate operation layer exposes only primitive candidate identity and extraction operations. JPEG headers, coefficient maps, Huffman state, F5 objects, and block sets remain private to `stegoeggo-stego`.
...
- **Application stego adapter** (`src/protected/steganography/`): `marker.rs` prepares current payloads, `embed.rs` dispatches carrier operations, `extract.rs` owns seed/search orchestration, `verify.rs` classifies payload integrity/authentication, and `legacy.rs` isolates V1/V2 compatibility; `mod.rs` is the facade and shared contract surface
- **Generic carrier core** (`stegoeggo-stego/src/`): Application-neutral LSB and JPEG DCT carrier mechanics, raw operations, and self-describing framed convenience operations — see `architecture/protected-steganography.md`
