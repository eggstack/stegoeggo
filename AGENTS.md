...
- **JPEG stego redundancy bug** (`steganography.rs:788-841`): Only first pass embeds when `redundancy > 1`. `embedded` counter not reset between passes. **FIXED**: Add `embedded = 0;` after line 840 (see `plans/plan.md` item 1A.1)
