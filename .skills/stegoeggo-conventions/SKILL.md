...
- `stegoeggo version` is offline and prints exactly `stegoeggo X.Y.Z` first. `stegoeggo update` trusts the latest non-yanked stable `stegoeggo-cli` crates.io version, verifies the matching GitHub asset and sidecar plus candidate identity/version, checks replaceability before download, and never invokes `sudo`. Cargo fallback is limited to unsupported targets or exact-asset HTTP 404; staged failures preserve the current executable.
