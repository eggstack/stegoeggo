...
- The release workflow stages the exact Cargo output path `target/<triple>/release/stegoeggo` (or `stegoeggo.exe` on Windows), copies it to the distinct public asset name, and never uses broad filesystem discovery. `scripts/check-docs-contract.sh` checks this source-path and matrix contract on the required CI path.
...
- `stegoeggo version` is offline and prints exactly `stegoeggo X.Y.Z` first. `stegoeggo update` queries the crates.io stable version first through embedded eggfetch-core, reports current without destination preflight when already current, otherwise checks replaceability before downloading any update asset (never before the registry lookup), verifies the matching GitHub asset and sidecar plus candidate identity/version, and never invokes `sudo`. Cargo fallback is limited to unsupported targets or exact-asset HTTP 404; staged failures preserve the current executable.
