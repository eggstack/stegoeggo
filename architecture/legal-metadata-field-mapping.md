...
| Contact email | `contact_email` | `Contact` tEXt | `Contact:` COM | **Not injected** | `Contact` tEXt → `contact`, `Contact:` COM → `contact`; WebP: **not extracted** (`contact` hard-coded `None`) | IPTC `Iptc4xmpExt:CreatorContactInfo` (not used) | **WebP round-trip broken**: writer never injects `contact_email` into WebP XMP. `photoshop:Credit` is written but sourced from `credit_line`, not `contact_email`. Extraction maps `photoshop:Credit` to `credit_line`; `contact` stays hard-coded `None`. |
| Credit line | `credit_line` | `CreditLine` tEXt | `CreditLine:` COM | `photoshop:Credit` | `CreditLine` tEXt → `credit_line`, `CreditLine:` COM → `credit_line`, `photoshop:Credit` → `credit_line` (WebP) | `photoshop:Credit` | Round-trips in all three formats. |
| Copyright owner | `copyright_owner` | `CopyrightOwner` tEXt | `CopyrightOwner:` COM | `stegoeggo:CopyrightOwner` | Same keys per format → `copyright_owner` | Custom `stegoeggo:` namespace | Round-trips in all three formats. |
| Licensor name/email/url | `licensor_name`, `licensor_email`, `licensor_url` | `LicensorName`/`LicensorEmail`/`LicensorURL` tEXt | Same with `:` COM suffix | `stegoeggo:LicensorName`/`LicensorEmail`/`LicensorURL` | Same keys per format → `licensor_*` | Custom `stegoeggo:` namespace | Round-trip in all three formats. |
| Metadata date | `metadata_date` | `MetadataDate` tEXt | `MetadataDate:` COM | `xmp:MetadataDate` | Same keys per format → `metadata_date` | `xmp:MetadataDate` | Round-trips in all three formats. |
| Notice applied at | `notice_applied_at` | `NoticeAppliedAt` tEXt | `NoticeAppliedAt:` COM | `stegoeggo:NoticeAppliedAt` | Same keys per format → `notice_applied_at` | Custom `stegoeggo:` namespace | Round-trips in all three formats. |
| Usage-terms language | `usage_terms_lang` | — (attribute on `UsageTerms`) | — (attribute on `UsageTerms:`) | `xml:lang` on `xmpRights:UsageTerms` `rdf:li` (defaults `x-default`) | **Not extracted** | — | Write-only language tag; extraction recovers `usage_terms` text only. |
