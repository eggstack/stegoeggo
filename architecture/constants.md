...
**Source:** `src/protected/constants.rs` (~8 lines), `stegoeggo-stego/src/constants.rs`, and the application adapter under `src/protected/steganography/`
...
| `SPLITMIX64_SEED` | `0x9e3779b97f4a7c15` | `stegoeggo-stego/src/constants.rs` (carrier mixer); `src/protected/steganography/mod.rs` holds a test-only copy of the same literal | Splitmix64 mixing constant |
...
| `MIN_PAYLOAD_SIZE` | `28` | `protected/steganography/mod.rs` (private `const`) | Parsing threshold (not output size) |
| `V3_PAYLOAD_VERSION` | `3` | `payload_v3/types.rs` | Current payload format version |

## Design Notes

- `STEGO_OFFSET_SEED_1` is a large prime-like constant used in the seed derivation formula for legacy offset seeds: `offset_seed = seed.wrapping_mul(STEGO_OFFSET_SEED_1.wrapping_add(pass))` (see `src/protected/steganography/extract.rs`). The corrected V2 carrier uses the raw seed directly without this offset
